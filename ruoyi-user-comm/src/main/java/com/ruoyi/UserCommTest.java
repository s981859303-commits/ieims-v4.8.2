 ...
 */
@Component
public class UserCommTest {
    @Autowired
    private RedisCache redisCache;
    @Autowired
    private RedisUtil redisUtil;

    @PostConstruct // 此时 redisUtil 已被成功注入
    public void testRedis2() {
        redisUtil.set("test2:11",new Date());
        redisUtil.set("test1:a","asdfasdfsa");
        redisUtil.set("test2:20",new Date(),20,TimeUnit.SECONDS);
    }
    /**
     * 执行所有 Redis 操作示例
     */
